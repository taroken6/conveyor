...
var getFieldOrderAlternate = function getFieldOrderAlternate(_ref) {
  ...
  console.log('===', modelName, fieldName, fieldOrder); // get form object from redux store; 'fieldName' indicates location

  var fieldOrderValues = fieldName ? R.pathOr([], [modelName, 'fields', fieldName, 'fieldOrderAlt'], tableView) : R.pathOr([], [modelName, 'fieldOrderAlt'], tableView); // get fieldNames from form object

  var alt = R.map(function (val) {
    return val.value;
  }, fieldOrderValues); // if the result is empty, return 'fieldOrder' as default

  alt = R.isEmpty(alt) ? fieldOrder : alt; // return alternate fieldOrder list
...
var FieldOrderAlterButton = function FieldOrderAlterButton(_ref2) {
  var active = _ref2.active,
      ...
      fieldOrderValues = _ref2.fieldOrderValues,
...
var FieldOrderAlterDetail = function FieldOrderAlterDetail(_ref3) {
  ...
  var fieldOrderValues = R.path([modelName, 'fields', fieldName, 'fieldOrderAlt'], tableView);
  return _react["default"].createElement(FieldOrderAlter, {
    schema: schema,
    modelName: modelName,
    fieldName: fieldName,
    fieldOrder: fieldOrder,
    fieldOrderValues: fieldOrderValues,
...
var FieldOrderAlterIndex = function FieldOrderAlterIndex(_ref4) {
  ...
  var fieldOrderValues = R.path([modelName, 'fieldOrderAlt'], tableView);
  return _react["default"].createElement(FieldOrderAlter, {
    schema: schema,
    modelName: modelName,
    fieldOrder: fieldOrder,
...
var FieldOrderAlter = function FieldOrderAlter(_ref5) {
  var schema = _ref5.schema,
      ...
      fieldOrderValues = _ref5.fieldOrderValues,
  ...
  var active = fieldOrderValues && !R.isEmpty(fieldOrderValues); // todo: fix this it doesn't always get right name
